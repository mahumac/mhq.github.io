...
通过在本地部署一套 Loki 日志系统，将华为/H3C/Juniper 等网络设备的日志集中存储，并使用 Grafana 进行日志管理查询。
...
如果对syslog协议 和 rsyslog软件不了解的话，就无法对日志进行格式化，方便Loki对日志流进行标签索引。
...
| 设备                  | 设备名称                 | IP              |
| --------------------- | ------------------------ | :-------------- |
| 日志服务器（rsyslog） | loki                     | 192.168.100.100 |
...
| 防火墙                | GZ-DX-FW-USG12000-01     | 192.168.100.3   |

说明：

设备名 `GZ-DX-CE-MX10003-01`，是一台位于广州电信机房的Juniper路由器，作为PE(ProviderEdge)，型号MX10003。
...
==命名规范相当重要，后面的 rsyslog 脚本，loki标签，告警规则 都需要根据设备 名称/型号/层级 来标记分类==。
...
-limits.per-user-override-config=<filename> # flag已经弃用，该功能重命名为“-runtime-config.file” (runtime_config.file in YAML)
```
...
- job_name: device-huawei
  ...
  - match:
      ...
      stages:
      - regex:
          # regex 阶段解析出PRI、Host app Severity等标签，这几个值只为 pipeline 内部设置，在以后续阶段可以使用这些值并决定如何处理他们。
          expression: '<(?P<pri_text>\d{3}|(?P<facility>\w*)\.(?P<severity>\w*))> (?P<time>[^ ]+) (?P<host>[^ ]*) (?P<app>[^ \/]*%%01\S+)\/(?P<level>[0-9])\/(?P<brief>[^ ":\/\(\)\[\]]*)(\(\w*\)|\[\w*\])*:*\s*(?P<message>.*)$'
        # labeLs阶段从前面的 regex 阶段获取PRI、Host、app、Severity 值，并将他们变成一个标签，比如GZ-DX-PE-MX10003就是这个阶段添加的一个标签。    
      - labels:
          facility:
          severity:
          host:
...
- job_name: device-juniper
  ...
  - match:
      ...
      # labeLs阶段从前面的 regex 阶段获取PRI、Host、app、Severity 值，并将他们变成一个标签。
      - labels:
          facility:
          severity:
          host:
...
server {
 ...
 location / {
     ...
     auth_basic_user_file   auth/htpsswd;     # 存放 usernmae/passwd 的文件
