...
如果对 [syslog协议]([syslog 协议简介 | My Blog](https://mahumac.github.io/posts/syslog-协议简介/)) 和 [rsyslog软件]([rsyslog配置详解 | My Blog](https://mahumac.github.io/posts/rsyslog配置详解/))不了解的话，就无法对日志进行格式化，方便Loki对日志流进行标签索引, 建议阅读本文之前，先去了解下。
...
![huawei-logformat](../image/2024-04-10-使用-Loki-日志系统来集中存储和查询网络设备日志.assets/huawei-logformat-1.png)
...
刚好 fluentbit 提供了RFC 4164 、RFC 5414的正则表达式
...
对比 RFC 3164 缺少了最前面的`PRI`部分（`Facility`+`Severity `），`Severity `（严重性级别 = 4）位置不对。原因是通 console或terminal查看设备日志的时候， `PRI`部分只是不显示而已，将日志发送到远程日志服务器的时候，就会带上`PRI`部分。

在日志服务器上抓包，可以看到：

![image-20241102082508227](../image/2024-04-10-使用-Loki-日志系统来集中存储和查询网络设备日志.assets/huawei-logformat-2.png)
...
参考 fluentbit 的正则表达式，华为设备的日志格式可以使用以下正则来解析:

```
Name    syslog-huawei
Regex   ^<(?P<facility>\w*)\.(?P<severity>\w*))> (?P<time>[^ ]+) (?P<host>[^ ]*) (?P<app>[^ \/]*%%01\S+)\/(?P<level>[0-9])\/(?P<brief>[^ ":\/\(\)\[\]]*)(\(\w*\)|\[\w*\])*:*\s*(?P<message>.*)$'
```

可以打开 https://regex101.com，来调试正则表达式是否正确。我手头没有其他厂商 （H3C/Cisco/锐捷等）的设备，正则表达式我就不提供了，需要的可以参考 syslog-huawei 自己去写正则解析。
...
- 受 Prometheus 启发，使用标签对日志流进行索引和分组
...
# 动态文件 模板
template(
    name="DynaFileTemplate-Core-Huawei" type="string"           # <<< Core-华为 设备的日志文件保存位置
    string="/var/log/network/core/huawei/%HOSTNAME%-%FROMHOST-IP%.log"
)

template(
    name="DynaFileTemplate-Storage-Huawei" type="string"        # <<< Storage-华为 设备的日志文件保存位置
    string="/var/log/network/storage/huawei/%HOSTNAME%-%FROMHOST-IP%.log"
)

template(
    name="DynaFileTemplate-Access-Huawei" type="string"			# <<< Access-华为 设备的日志文件保存位置
    string="/var/log/network/access/huawei/%HOSTNAME%-%FROMHOST-IP%.log"
)

template(
    name="DynaFileTemplate-Firewall-Huawei" type="string"       # <<< Firewall-华为 设备的日志文件保存位置
    string="/var/log/network/firewall/huawei/%HOSTNAME%-%FROMHOST-IP%.log"
)

template(
    name="LogFormat-Juniper"  type="string"                     # <<< Juniper 设备的日志文件保存位置
    string="<%PRI-text%> %TIMESTAMP:::date-rfc3339% %HOSTNAME% %APP-NAME% %PROCID% %MSGID% %STRUCTURED-DATA% %msg%\n"
)
template(
    name="DynaFileTemplate-default-other" type="string"         # <<< 匹配不到命名规则 的设备日志文件保存位置
...
template(
    name="LogFormat-rawmsg"  type="string"                 # <<< 日志格式保留为原始，（调试使用）
    string="<%rawmsg%\n"
)
template(
    name="LogFormat-rawmsg-after-pri"  type="string"       # <<< 为原始日志格式添加 pri头，（调试使用）
    string="<%pri-text%>%rawmsg-after-pri%\n"
)
template(
    name="LogFormat-Huawei"  type="string"     # <<< 华为 设备的日志格式，近似rfc3164
    string="<%pri-text%> %timereported:::date-rfc3339% %HOSTNAME% %syslogtag% %msg:::sp-if-no-1st-sp%%msg:::drop-last-lf%\n"
)
template(
    name="LogFormat-Juniper"  type="string"    # <<< Juniper 设备的日志格式，近似rfc5414
    string="<%PRI-text%> %TIMESTAMP:::date-rfc3339% %HOSTNAME% %APP-NAME% %PROCID% %MSGID% %STRUCTURED-DATA% %msg%\n"
)
...
上面配置文件中的 `$hostname `、`$syslogtag`是rsyslog内置的变量。更多细节需要了解下 syslog协议 和 rsyslog软件。
